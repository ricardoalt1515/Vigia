...
- [ ] **[TEST FIRST]** Add `internal/harness/risk_test.go` — table-driven test asserting the three `RiskClass` constants (`read`, `draft`, `authority`) exist, are of type `RiskClass`, and are mutually distinct.
...
- [ ] Create `internal/harness/risk.go` — declare `RiskClass` type (string alias) and three constants: `RiskClassRead`, `RiskClassDraft`, `RiskClassAuthority`.
  - No logic, no imports beyond the bare minimum.
  - Constraint: do not modify any existing file in `internal/harness/`.

### Verify WU1
- [ ] Run `go test ./internal/harness` — all tests green including new risk_test.go.
...
- [ ] Create directories `internal/harness/labtools/fixtures/cases/` and `internal/harness/labtools/fixtures/rules/`.
...
- [ ] Create `internal/harness/labtools/fixtures/cases/case-001.json` — synthetic Case fixture. Must carry all required fields non-empty:
...
- [ ] Create `internal/harness/labtools/fixtures/rules/rule-mx-redeco-04.json`:
  - `code`: `MX-REDECO-04`
...
- [ ] Create `internal/harness/labtools/fixtures/rules/rule-mx-redeco-05.json`:
  - `code`: `MX-REDECO-05`
...
- [ ] Create `internal/harness/labtools/fixtures.go` (package `labtools`) — declare Go structs mirroring the JSON shape:
...
- [ ] Create `internal/harness/labtools/embed.go` (package `labtools`):
...
- [ ] **[TEST FIRST]** Create `internal/harness/labtools/loader_test.go` — all tests must fail before `loader.go` exists. Cover:
...
- [ ] Create `internal/harness/labtools/loader.go` — implement `Load() (CaseStore, RuleStore, error)`:
...
### Verify WU2
- [ ] Run `go test ./internal/harness/labtools` — all loader tests green.
- [ ] Run `go test ./...` — no regressions.
