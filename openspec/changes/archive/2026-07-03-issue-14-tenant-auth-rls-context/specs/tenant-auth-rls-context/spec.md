...
Define the minimal issue #14 runtime proof that a tenant API key can authenticate a request, establish transaction-scoped tenant context for PostgreSQL row-level security, and protect one product-data endpoint without expanding into the broader walking skeleton.
